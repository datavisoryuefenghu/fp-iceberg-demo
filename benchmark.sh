...
# --- Produce events ---
echo "=== StarRocks Benchmark (event_result with resolved features) ==="
...
echo "Warming up StarRocks (first query is slow due to metadata/S3 cache cold start)..."
...
for i in "${!QUERY_NAMES[@]}"; do
  name="${QUERY_NAMES[$i]}"
  read -r avg min max <<< "$(bench "${QUERIES[$i]}")"
  printf "%-30s %20s\n" "$name" "$avg ($min-$max)"
done
